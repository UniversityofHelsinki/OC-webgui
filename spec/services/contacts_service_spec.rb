RSpec.describe ContactsService, type: :service do
	...
	context "when database contains some agent statuses that corrsepond to a contact" do 
		...
		before (:example) do
			time = Time.parse("#{Date.today} 08:00:00")
		...
		it "creates Contact objects of those objects and returns them correctly" do
			contacts = ContactsService.new.contacts_for_team('Helpdesk', "#{Date.today} 00:00:00", "#{Date.today} 23:59:59")
		...
		it "sets the call start and end times according to status creatiion and closing times" do 
			contacts = ContactsService.new.contacts_for_team('Helpdesk', "#{Date.today} 08:00:00", "#{Date.today} 09:00:00")
			expect(contacts[0].answered).to eq("#{Date.today} 05:05:00")
			expect(contacts[0].call_ended).to eq("#{Date.today} 05:25:00")
			expect(contacts[0].agent_id).to eq(123)

			expect(contacts[1].answered).to eq("#{Date.today} 05:25:00")
			expect(contacts[1].call_ended).to eq("#{Date.today} 05:35:00")
			expect(contacts[1].agent_id).to eq(225)
		end

		it "correctly accounts for after call status occurring right after the contact" do
			contacts = ContactsService.new.contacts_for_team('Helpdesk', "#{Date.today} 08:00:00", "#{Date.today} 08:06:00")
			expect(contacts[0].handling_ended).to eq("#{Date.today} 05:44:00")
		end

		it "ignores after call status which might be connected to a different contact" do
			contacts = ContactsService.new.contacts_for_team('Helpdesk', "#{Date.today} 10:00:00", "#{Date.today} 11:06:00")
			expect(contacts[0].handling_ended).to be(nil)
			expect(contacts[1].handling_ended).to be(nil)
		end

=begin		it "returns the correct amount of hours for each specified time period" do
			expect(ContactsService.new.get_contacts_between("08:00:00", "08:59:59")).to eq(2)
			expect(ContactsService.new.get_contacts_between("09:00:00", "09:59:59")).to eq(5)
			expect(ContactsService.new.get_contacts_between("10:00:00", "10:59:59")).to eq(2)
			expect(ContactsService.new.get_contacts_between("11:00:00", "11:59:59")).to eq(3)
			expect(ContactsService.new.get_contacts_between("12:00:00", "12:59:59")).to eq(3)
			expect(ContactsService.new.get_contacts_between("13:00:00", "13:59:59")).to eq(4)
			expect(ContactsService.new.get_contacts_between("14:00:00", "14:59:59")).to eq(1)
			expect(ContactsService.new.get_contacts_between("15:00:00", "15:59:59")).to eq(2)
		end
=end
	end
